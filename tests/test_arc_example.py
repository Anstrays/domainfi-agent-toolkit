...
class ArcX402PaidAgentExampleTests(unittest.TestCase):
    ...
    def test_scan_requires_payment_then_returns_paid_opportunities(self) -> None:
        ...
        self.assertEqual(challenge["payment_intent"]["kind"], "arc_testnet_payment_intent")
        self.assertIn("domainfi_arc_payment_verify", {tool["name"] for tool in challenge["mcp_tools"]["tools"]})

        status, payload = self._get_json(payment="x402-test:domainfi.discovery.scan:25000")
        self.assertEqual(status, 200)
        self.assertTrue(payload["payment"]["paid"])
        self.assertTrue(payload["payment_receipt"]["paid"])
        self.assertEqual(payload["payment_receipt"]["status"], "accepted")
